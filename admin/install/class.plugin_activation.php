...
if ( ! class_exists( 'IncipioPluginActivation' ) ) {
	/**
	...
	class IncipioPluginActivation {
		 ...
		 * @since 1.0.0
		 *
		 * @see IncipioPluginActivation::init()
		 */
		 ...
		 * @since 2.0.0
		 *
		 * @see IncipioPluginActivation::admin_menu()
		 * @see IncipioPluginActivation::notices()
		 * @see IncipioPluginActivation::styles()
		 */
		 ...
		 *
		 * @see IncipioPluginActivation::init()
		 * @see IncipioPluginActivation::install_plugins_page()
		 */
...
if ( ! function_exists( 'tgmpa' ) ) {
	...
	function tgmpa( $plugins, $config = array() ) {
		...
		foreach ( $plugins as $plugin )
			IncipioPluginActivation::$instance->register( $plugin );

		if ( $config )
			IncipioPluginActivation::$instance->config( $config );
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		 ...
		 * and stores it in the global object IncipioPluginActivation::$instance.
		 *
		 * @since 2.2.0
		 *
		...
		protected function _gather_plugin_data() {
			...
			IncipioPluginActivation::$instance->admin_init();
			IncipioPluginActivation::$instance->thickbox();
			...
			foreach ( IncipioPluginActivation::$instance->plugins as $plugin ) {
				if ( is_plugin_active( $plugin['file_path'] ) )
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = __( 'External Link', IncipioPluginActivation::$instance->domain );
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = __( 'Private Repository', IncipioPluginActivation::$instance->domain );
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = __( 'Pre-Packaged', IncipioPluginActivation::$instance->domain );
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = __( 'WordPress Repository', IncipioPluginActivation::$instance->domain );
				}

				$table_data[$i]['type'] = $plugin['required'] ? __( 'Required', IncipioPluginActivation::$instance->domain ) : __( 'Recommended', IncipioPluginActivation::$instance->domain );

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', IncipioPluginActivation::$instance->domain ) );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', IncipioPluginActivation::$instance->domain ) );
		...
		protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
			...
			foreach ( IncipioPluginActivation::$instance->plugins as $plugin => $values ) {
				if ( $name == $values['name'] && isset( $values[$data] ) )
					return $values[$data];
			}
				...
				$actions = array(
					'install' => sprintf(
						...
						wp_nonce_url(
							add_query_arg(
								array(
									'page'          => IncipioPluginActivation::$instance->menu,
									'plugin'        => $item['slug'],
									'plugin_name'   => $item['sanitized_plugin'],
									'plugin_source' => $item['url'],
									'tgmpa-install' => 'install-plugin',
								),
								admin_url( IncipioPluginActivation::$instance->parent_url_slug )
							),
							'tgmpa-install'
						),
		...
		public function no_items() {
			...
			printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', IncipioPluginActivation::$instance->domain ), admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', IncipioPluginActivation::$instance->domain ),
				'source' => __( 'Source', IncipioPluginActivation::$instance->domain ),
				'type'   => __( 'Type', IncipioPluginActivation::$instance->domain ),
				'status' => __( 'Status', IncipioPluginActivation::$instance->domain )
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', IncipioPluginActivation::$instance->domain ),
				'tgmpa-bulk-activate' => __( 'Activate', IncipioPluginActivation::$instance->domain ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() ) {
				...
				if ( is_wp_error( $api ) )
					wp_die( IncipioPluginActivation::$instance->strings['oops'] . var_dump( $api ) );
				...
				$url   = add_query_arg( array( 'page' => IncipioPluginActivation::$instance->menu ), admin_url( IncipioPluginActivation::$instance->parent_url_slug ) );
				$nonce = 'bulk-plugins';
				$names = $plugin_names;
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( is_wp_error( $activate ) )
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, IncipioPluginActivation::$instance->domain ), $imploded );
 ...
 * @since 2.2.0
 */
if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET[sanitize_key( 'page' )] ) && IncipioPluginActivation::$instance->menu = $_GET[sanitize_key( 'page' )] ) ) {
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function bulk_install( $packages ) {
				...
				if ( IncipioPluginActivation::$instance->is_automatic )
			...
			public function run( $options ) {
				...
				if ( IncipioPluginActivation::$instance->is_automatic ) {
					...
					IncipioPluginActivation::$instance->populate_file_path();
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = __( 'Install package not available.', IncipioPluginActivation::$instance->domain );
				$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', IncipioPluginActivation::$instance->domain );
				$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', IncipioPluginActivation::$instance->domain );
				$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', IncipioPluginActivation::$instance->domain );
				$this->strings['process_failed']      = __( 'Plugin install failed.', IncipioPluginActivation::$instance->domain );
				$this->strings['process_success']     = __( 'Plugin installed successfully.', IncipioPluginActivation::$instance->domain );
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  = __( 'Plugin activation failed.', IncipioPluginActivation::$instance->domain );
				$this->strings['activation_success'] = __( 'Plugin activated successfully.', IncipioPluginActivation::$instance->domain );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( IncipioPluginActivation::$instance->is_automatic ) {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', IncipioPluginActivation::$instance->domain );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', IncipioPluginActivation::$instance->domain ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', IncipioPluginActivation::$instance->domain ) . '</span><span class="hidden">' . __( 'Hide Details', IncipioPluginActivation::$instance->domain ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', IncipioPluginActivation::$instance->domain );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', IncipioPluginActivation::$instance->domain );
				}
				/** Default installation strings */
				else {
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', IncipioPluginActivation::$instance->domain );
					$this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', IncipioPluginActivation::$instance->domain );
					$this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', IncipioPluginActivation::$instance->domain );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', IncipioPluginActivation::$instance->domain ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', IncipioPluginActivation::$instance->domain ) . '</span><span class="hidden">' . __( 'Hide Details', IncipioPluginActivation::$instance->domain ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', IncipioPluginActivation::$instance->domain );
					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', IncipioPluginActivation::$instance->domain );
				}
			...
			public function bulk_footer() {
				...
				foreach ( IncipioPluginActivation::$instance->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						echo '<p><a href="' . add_query_arg( 'page', IncipioPluginActivation::$instance->menu, admin_url( IncipioPluginActivation::$instance->parent_url_slug ) ) . '" title="' . esc_attr( IncipioPluginActivation::$instance->strings['return'] ) . '" target="_parent">' . __( IncipioPluginActivation::$instance->strings['return'], IncipioPluginActivation::$instance->domain ) . '</a></p>';
						$complete[] = $plugin;
						break;
					}
					/** Nothing to store */
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( IncipioPluginActivation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', IncipioPluginActivation::$instance->domain ) . '">' . __( 'Return to the Dashboard', IncipioPluginActivation::$instance->domain ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
